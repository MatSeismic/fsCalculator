...
class Engine {
  ...
  putDataToDB = (message) => {
    let currentIds = this.data.map((data) => data.id);
    let idToBeAdded = 0;
  ...
  handleEqualOperation(input) {
    if (this.previousNumber == null) {
      ...
      if (
        ...
        this.message = this.previousNumber + this.previousOperation + temp;
        ...
        this.message = this.message + "=" + this.number;

        this.putDataToDB(this.message);
